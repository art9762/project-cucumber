...
Управление сбором (collection) из панели: ручной запуск и расписание.

### Added

- **ui** — новая страница **Collect**: ручной запуск сбора по каждому источнику
  с поллингом статуса прогона и редактор cron-расписания по источникам
  (только для admin).
- **engine** — персистентное cron-расписание по источникам (таблица
  `source_schedules`, миграция `0002`); API `GET/PUT /schedule` с валидацией
  cron; `start()` накатывает сохранённые расписания поверх env-дефолтов.
- **analysis** — admin-guarded шлюз `/collect/*`, проксирующий в внутренний
  collector (sources, jobs, schedule). Collector наружу не выставляется — доступ
  только через аутентифицированный шлюз.

[1.0.2]: https://github.com/art9762/project-cucumber/releases/tag/v1.0.2
